...
export async function isRepoOwner(
  ...
  try {
    ...
    throw new GitHubError(
      `Failed to check repository ownership: ${error.message}`,
      "REPO_OWNER_CHECK_FAILED"
    );
  }
}
...
export async function mergePullRequest(
  ...
  try {
    ...
    // 409エラー: Head refが削除されている
    if (error.status === 409) {
      return {
        success: false,
        message: "PR head ref no longer exists",
      };
    }
